...
const addValues = (arr) => {
  let newValue = arr.reduce( (addedNum, num) => {
    addedNum = num + addedNum
...
const countNumberOfElements = (arr) => {
  let arrCount = arr.reduce( (count, value) => {
    count++
    return count 
  }, 0)
  return arrCount
...
const returnNames = (arr) => {
  let names = arr.reduce( (acc, person, idx) => {
    acc[idx] = person.name
    return acc
  }, [])
  return names;
...
const reversedString = (str) => {
  ...
  let newArr = stringArr.reduce( (acc, value) => {
    return value + acc
  }, '')
  return newArr;
};
