...
const count = (target, input) => {
  let numbers = input.map(arr =>arr.reduce( (acc, number, idx) => {
    if(target === number){
      acc = acc + 1
    }
    return acc
  ...
  let numberCount = numbers.reduce((acc, value) => {
    if(value > 0){
      acc = acc + value
    }
    return acc
...
const totalSum = (input) => {
  let count = input.reduce( (totalCount, value) => {
    totalCount = totalCount + value.reduce( (acc, int) => {
      acc = acc + int
      return acc
    }, 0)
    return totalCount
...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArray = input.map(arr => arr.filter( value => typeof(value) === 'number' && value % 5 === 0 ).map(int => Math.pow(2,int)))
...
let findMaleAndFemale = (data) => {
  let foundPeople = data.filter(value => value.gender === 'male' || value.gender === 'female').map( person => person.name).join(' and ')
  return foundPeople
};
...
let findShortest = (data) => {
  let shortest = data.sort((a,b) => Number(a.height) - Number(b.height))
  return shortest[0].name

  // let shortest = data.reduce( (acc, value, idx) => {
  //   if(value.height < acc){
  //     acc = value.height
  //   }else {
  //     acc = value.name
