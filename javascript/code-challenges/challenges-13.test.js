...
const longestString = (arr) => {
  let longest = arr.indexOf(arr.reduce((a, b) => {
...
const standardizePhoneNumbers = (arr) => {
  let newArr = arr.map(string => `${string.substr(1, 3)}${string.substr(6, 3)}${string.substr(10, 4)}`)
  return newArr
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 === 1) {
      let oddletter = str[i]
      console.log(oddletter)
      return oddString + oddletter
    }
...
const allHappy = (arr) => {
  let newArr = arr.every(value => value.includes(':)'))
  return newArr
};
